...
\hline
\rowcolor[HTML]{CCCCCC} \multicolumn{2}{|l|}{\textbf{my function}} \\
a & bar \\
\textit{Returns:} & bar \\
\hline
\end{tabular}
...
\chapter{./src/dict.txt}
\label{srcdicttxt}

Is a file containing a dictionary that I use to construct prototype tables for procedures in my code. You will probably not need or want it, so just add a \texttt{./src/dict.txt} containing the text "\texttt{foo -> bar}" or something like that (or just take the \texttt{./src/dict.txt} from the repository). With some editing of the scripts, the use for it can be removed. I still need it, so I'm not going to remove it myself.
